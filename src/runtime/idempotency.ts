...
function resolve(options: IdempotencyOptions): ResolvedOptions {
  if (!options || !options.adapter) {
    ...
    throw new Error(
      "withIdempotency requires an adapter. Import one from @wamasoda/nextdoc/idempotency/memory, /redis or /postgres.",
    );
  }
  return {
...
export function createIdempotency(options: IdempotentRunOptions) {
  ...
  if (!adapter) {
    throw new Error(
      "createIdempotency requires an adapter. Import one from @wamasoda/nextdoc/idempotency/memory, /redis or /postgres.",
    );
  }
